...
import { handleZodError, feedCommentUpdateSchema } from "@/lib/validations";
...
export async function PUT(
  ...
  try {
    ...
    if (existing.authorId !== alumni.id) {
      return NextResponse.json({ error: "คุณไม่สามารถแก้ไขความคิดเห็นของผู้อื่นได้" }, { status: 403 });
    }
    ...
    console.error("PUT /api/feed/comments/[id] error:", error);
...
export async function DELETE(
  ...
  try {
    ...
    if (who.staff) {
      ...
      return NextResponse.json({ error: "คุณไม่สามารถลบความคิดเห็นของผู้อื่นได้" }, { status: 403 });
    }

    await prisma.$transaction(async (tx) => {
      await tx.feedComment.update({ where: { id }, data: { deletedAt: new Date() } });
      await tx.feedPost.update({ where: { id: existing.postId }, data: { commentCount: { decrement: 1 } } });
    });
