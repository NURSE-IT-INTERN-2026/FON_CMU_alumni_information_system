...
export async function POST(request: NextRequest) {
  try {
    ...
    const records: { studentId: string; fullName: string; career: string; position: string; recordedYear: number }[] = [];
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!studentId || !fullName || !career || !position || !recordedYearStr) {
      ...
      records.push({ studentId, fullName, career, position, recordedYear });
    }

    let imported = 0;
    if (records.length > 0) {
      const result = await prisma.potential.createMany({ data: records });
