...
import { associationToExportRow } from "@/lib/association-excel";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const rows = items.slice(start - 1, end).map(associationToExportRow);
    await logActivity(
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rows = items.map(associationToExportRow);
