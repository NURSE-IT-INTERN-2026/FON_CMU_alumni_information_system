...
import { parseCommitteeRow, type CommitteeImportRecord } from "@/lib/graduate-committee-excel";

const MAX_IMPORT_FILE_SIZE = 5 * 1024 * 1024; // 5MB

/** A parsed graduate-committee row with its resolved alumni link. */
type Resolved = CommitteeImportRecord & {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const raw: CommitteeImportRecord[] = [];
    for (let i = 0; i < rows.length; i++) {
      const { data, error } = parseCommitteeRow(rows[i], i + 2);
      if (error) {
        errors.push(error);
        continue;
      }
