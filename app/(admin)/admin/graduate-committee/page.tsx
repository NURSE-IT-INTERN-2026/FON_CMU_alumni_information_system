...
export default function GraduateCommitteeAdminPage() {
  const [committees, setCommittees] = useState<GraduateCommittee[]>([]);
  ...
  const fetchFilterOptions = useCallback(async () => {
    try {
      ...
      const all: GraduateCommittee[] = json.data;

      const cohorts = [...new Set(all.map((c) => c.cohort).filter(Boolean))].sort();
      const positions = [...new Set(all.map((c) => c.position).filter(Boolean))].sort();
  ...
  }, [page, search, filterCohort, filterPosition]);
  ...
  const openEdit = (c: GraduateCommittee) => {
    setForm({
      ...
      <div className="rounded-xl bg-white shadow-sm border border-gray-100 p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <select
            ...
            className="rounded-lg border border-gray-300 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white min-w-[140px]"
          >
          ...
          <select
            ...
            className="rounded-lg border border-gray-300 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white min-w-[160px]"
          >
              ...
              {loading ? (
                ...
                committees.map((c, i) => (
                  <tr key={c.id} className="hover:bg-blue-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3 font-mono text-gray-700">{c.studentId}</td>
          ...
          <div className="w-full max-w-lg rounded-2xl bg-white shadow-xl max-h-[90vh] flex flex-col">
            ...
            >
              <h2 className="text-lg font-semibold">
                {modalMode === "create" ? "เพิ่มข้อมูลกรรมการบัณฑิต" : "แก้ไขข้อมูลกรรมการบัณฑิต"}
              </h2>
              <button
                onClick={closeModal}
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              <Field label="ปี พ.ศ. *" error={formErrors.termYear}>
                <input
                  type="number"
                  value={form.termYear}
                  onChange={(e) => setForm((f) => ({ ...f, termYear: e.target.value }))}
                  placeholder="เช่น 2568"
                  className={inputClass(formErrors.termYear)}
                />
              </Field>
              ...
              <Field label="ชื่อ-สกุล (ขณะกำลังศึกษา) *" error={formErrors.fullName}>
                <input
                  type="text"
                  value={form.fullName}
              ...
              <Field label="รุ่นที่ *" error={formErrors.cohort}>
                <input
                  type="text"
                  value={form.cohort}
                  onChange={(e) => setForm((f) => ({ ...f, cohort: e.target.value }))}
                  className={inputClass(formErrors.cohort)}
                />
              </Field>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <p className="mb-6 text-sm text-gray-600">
              คุณต้องการลบข้อมูลกรรมการบัณฑิตนี้หรือไม่? การดำเนินการนี้ไม่สามารถย้อนกลับได้
            </p>
...
function Th({
  ...
  return (
    <th
      className={`px-4 py-3 text-xs font-semibold uppercase tracking-wider whitespace-nowrap ${className}`}
    >
      {children}
    </th>
  );
