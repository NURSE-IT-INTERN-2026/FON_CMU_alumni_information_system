        ...
        {canWrite && (selectMode ? (
          <div className="flex items-center gap-2">
            <button onClick={() => (isAllSelected(awards.map((a) => a.id)) ? deselectPage(awards.map((a) => a.id)) : selectAll(awards.map((a) => a.id)))} className="rounded-lg border border-[var(--border)] bg-white px-4 py-2 text-sm font-medium text-[var(--foreground)] transition-colors hover:bg-gray-50">
              {isAllSelected(awards.map((a) => a.id)) ? "ยกเลิกเลือกหน้านี้" : "เลือกทั้งหมดในหน้านี้"}
            </button>
            ...
            <div className="flex items-center gap-1 flex-wrap justify-center">
              ...
              {paginationNumbers.map((p, i) =>
                p === "..." ? <span key={`dot-${i}`} className="px-2 text-gray-400">...</span> : (
                  <button key={p} onClick={() => { setPage(p as number); }} className={`rounded-lg px-3 py-1.5 text-sm font-medium ${page === p ? "bg-[var(--primary)] text-white" : "text-gray-600 bg-white hover:bg-gray-100"}`}>{p}</button>
                )
              )}
