...
- **What:** The all-alumni page (`app/(admin)/management/all-alumni/page.tsx`) has two tables. **Both** fetch the FULL CMU list once (query keyed **without `page`** — only `search`/`filters`/`sort`; `pageSize=50000` since `/api/cmu-alumni` caps at `MAX_PAGE_SIZE=50000`) AND the full local list (`/api/alumni?pageSize=50000&includeDeleted=true` — the `50000` is load-bearing: there are >11k local alumni and the old `9999` silently **truncated ~1,600 records**), merge the complete set, then slice client-side (`allMerged.slice((page-1)*PAGE_SIZE, page*PAGE_SIZE)`) — so navigating pages is instant (no refetch). **Both modes now include local-only rows** (alumni whose `studentId` is NOT in CMU — admin-created or legacy-Excel-imported; ~4,167 as of 2026-06): after the CMU overlay, a secondary pass appends each one, mapped to the `CmuAlumni` shape with a `degreeLabel` overlay (the local `DegreeLevel` enum can't round-trip through CMU's `level_id` losslessly, so the label is carried directly; the degree column renders `a.degreeLabel || getLevelLabel(...)`). Without this pass, created/imported alumni are **invisible** in the table (the original bug). Both modes **sort client-side** with `sortAlumni(merged, field, dir)` (`lib/alumni-sort.ts`) — view mode maps its `ViewSortField` to a CmuAlumni property via `VIEW_SORT_FIELD_MAP` first, because the CMU proxy can't place local-only rows. Facets filter local rows server-side (mirrored from manage mode); search filters local-only rows client-side. `allMerged`/`allViewMerged` (the full set, not the page slice) is used for delete/bulk-delete lookups so CMU-only rows selected on other pages still resolve.
...
- **Trap:** Adding a sortable column to **either** table must keep it reachable by the client-side sort: manage sorts `Alumni`-shaped rows directly; view maps the `ViewSortField` → CmuAlumni property in `VIEW_SORT_FIELD_MAP` (add an entry there for a new view column — the view table also still sends `sortField` to the CMU proxy, but since view re-sorts client-side that only orders the CMU subset). Don't page the CMU proxy in **either** mode (one page at a time) and merge per-page — local-only rows would repeat across pages and the sort would be wrong. Fetch the **full** set once, merge, sort, then slice client-side (see above).
