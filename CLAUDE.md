...
- **Prevention:** Use `getPersonDegreeBreakdown()` (`lib/person-degree-count.ts`) in `/api/alumni-count` and `/api/dashboard`. Its pure core `groupPersonsByDegree(cmu, local)` unions entities via three signals — CMU records sharing normalized name+birthday, a local alumni's `education` rows (one `alumniId`), and a cross-source **studentId bridge** (a local education whose `studentId` matches a CMU record joins that CMU person). Each group → one person, highest degree (`DEGREE_RANK` from `lib/alumni-verify.ts`), representative year = that degree's most recent year. **The count is scoped to the CMU registrar universe** (matches the all-alumni table): each person is flagged `hasCmu`, and `getPersonDegreeBreakdown` counts only those — local education still UPGRADES a CMU person's degree, but a **local-only** person (no CMU record, e.g. a legacy/junk import like `studentId=1000000`) is excluded so it can't inflate the total above the registrar count. (Before the education backfill, local alumni had no `education` rows so `groupPersonsByDegree` skipped them and the graph silently matched the table; the backfill exposed ~4,160 local-only persons that had to be scoped out.) The table (`/api/cmu-alumni`) + facets still use `dedupeCmuGraduatesByPerson` (CMU-only). **Note:** alumni-count's `DEGREE_ORDER`/labels are now only used for response shaping, not degree resolution.
