...
- `/api/alumni-auth/signup` requires a `degreeLevel` select (client `alumniSignupSchema` + API `alumniSignupApiSchema`). **Admin-approval flow:** signup no longer verifies identity or degree (the old hard CMU/local gate + degree-mismatch 400 are REMOVED — the admin is the gatekeeper, using the `signupVerification` snapshot) and no longer auto-logins. It best-effort fetches CMU, stores a per-field `signupVerification` snapshot (`lib/signup-verification.ts` — ✓/✗/— per field + the raw submitted values, for re-verify), and creates a `PENDING` account. The snapshot has **two authoritative sources** in priority order (`buildSignupVerification(submitted, cmuGrad, cmuConsulted, local?)`): (1) the CMU Registrar record (preferred), (2) the **local `alumni` record** as a fallback when CMU has no record but the studentId already exists locally (an admin-created/legacy alumni, e.g. `66123456`) — `source: "cmu" | "local" | null` records which was used and the admin review modal (`VerificationFields` in `settings/users/page.tsx`) renders the differences table + a source label for both. Without this fallback every local-only signup showed a bare "ไม่พบรหัสนักศึกษา" with no comparison. `local` is only passed when a pre-existing alumni was found at signup (`localAuthoritativeFromAlumni`); reverify (`POST /api/alumni-accounts/[id]/reverify`) re-reads the alumni record as `local` ONLY when the stored `source === "local"` (so a freshly-created-from-submitted account never self-matches). (`accountStatus: PENDING`, `passwordHash`, `signupVerification`) — no session, no cookie, no `hasLoggedIn`, and **no Education row** (Education + `syncPrimarySnapshot` + graduation logs move to `POST /api/alumni-accounts/[id]/approve`, so a rejected signup never pollutes degree data). Returns `{ success, pending: true }`; the signup page shows a "pending review" success state. The alumni-portal + admin profile render educations as a responsive grid of colored cards (`components/EducationSection.tsx`, 1/row on narrow screens, 2/row wider) tinted with `DEGREE_COLORS` (`lib/constants.ts`, the same colors as the dashboard graph); each card has its own แก้ไข button.
