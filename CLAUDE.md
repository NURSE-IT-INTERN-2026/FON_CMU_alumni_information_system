...
31 models in `prisma/schema.prisma` (table names from `@@map`). Long descriptions live in the **Known Pitfalls** entries they point to — the cells here are terse pointers.
...
| `CommunityProfile` | `community_profiles` | Opted-in alumni's SELF-PUBLISHED community profile (V2) — 1:1 with `Alumni` (`alumniId @unique`, Cascade). Own photo (self-service upload; display falls back to `Alumni.photoUrl`), workplace/position/province/country, plain-text `bio` (≤2,000), + self-published contact (`contactEmail`/social links — NOT the private `Alumni.contactEmail`). Separate table ON PURPOSE: the extended fields can't leak through the forum identity select. Directory reads additionally filter `communityOptedInAt != null` (opt-out hides the profile without deleting it) |
| `CommunityGroup` | `community_groups` | V2 group/sub-community. A group's "space" is NOT a separate feed — it's the existing `ForumTopic`/`CommunityEvent` lists scoped by their nullable `groupId` FK (`SetNull`). `kind`: COHORT (auto-derived per cohort label via `ensureCohortGroup`, `lib/group-cohort.ts`; slug `cohort-<normalized>`; lazily created on join + seeded by `scripts/seed-cohort-groups.ts`) or INTEREST (API-created by staff/opted-in alum; creator becomes MODERATOR). `memberCount`/`topicCount` denormalized in `$transaction`s. Soft-delete (`community-group` in `TRASH_ENTITIES`; memberships cascade, topics/events survive unscoped) |
| `GroupMembership` | `group_memberships` | An alum's membership in a `CommunityGroup` — `@@unique([groupId, alumniId])`, `role` MEMBER/MODERATOR. Join is idempotent; `cohort:<label>` join slugs lazily materialize the cohort group (own-cohort only — 403 joining another cohort's group) |
| `JobPosting` | `job_postings` | V2 job-board posting. Broadcast read (any ACTIVE alum, like events); dual author FKs (`authorAlumniId` XOR `authorUserId`, both `SetNull`). Plain-text description; `applyUrl`/`contactInfo` INTENTIONALLY public. Required `expiresAt` (≤90 days, zod); lists hide expired by default (no cron). Soft-delete (`job-posting` in `TRASH_ENTITIES`); reportable (`JOB_POSTING`) |
| `EventPhoto` | `event_photos` | V2 event photo album — one row per uploaded photo (bytes via `/api/alumni-upload`, registered by POST `{imageUrl, caption}`). Upload gate: ATTENDING alum / alumni organizer / staff (`lib/event-photo-gate.ts`). `imageUrl` basePath-relative → render via `assetUrl`. Soft-delete; reportable (`EVENT_PHOTO`) |
| `Announcement` | `announcements` | V2 staff announcement — plain-text (NOT Tiptap; that's News), staff `authorUserId` (`SetNull`), `pinnedAt` floats to top, optional `expiresAt` hides. Broadcast audience (staff + every ACTIVE alum). "ใหม่" badge = per-alumni watermark `Alumni.announcementsLastReadAt` (NO notification fan-out). Soft-delete (`announcement` in `TRASH_ENTITIES`) |
| `Notification` | `notifications` | V2 in-app notification — the recipient alum, `NotificationType`, `entityId`, + PRE-RENDERED Thai `title`/`body`/`link` snapshots (source edits/deletes can't break the center). `readAt` null = unread. Emitters best-effort (`lib/notification-emitter.ts`) — a failed insert never fails the parent action. Read-notifications > 90d purged by `/api/auth/cleanup` |
...
- `ContentReportStatus`: OPEN, RESOLVED, DISMISSED · `ContentReportReason`: SPAM, HARASSMENT, INAPPROPRIATE, OTHER · `ForumReportResource`: FORUM_TOPIC, FORUM_REPLY, EVENT, FEED_POST, FEED_COMMENT, JOB_POSTING, EVENT_PHOTO (one report system across forum + events + feed + jobs + albums) · `RsvpStatus`: ATTENDING, DECLINED (community events) · `GroupKind`: COHORT, INTEREST · `GroupRole`: MEMBER, MODERATOR (V2 groups) · `NotificationType`: REPLY_TO_MY_TOPIC, LIKE_ON_MY_POST, COMMENT_ON_MY_POST, RSVP_ON_MY_EVENT, NEW_GROUP_TOPIC, REPORT_OUTCOME (V2)
...
│       ├── profile/              # Alumni self-profile (view/edit) + self-contained CommunityProfileSection card (community V2, view mode only)
...
│       ├── events/ + events/[id]/ + events/new/   # Alumni community events (broadcast; RSVP w/ guests+capacity; list/calendar view toggle)
│       ├── directory/ + directory/[id]/        # Alumni directory (community V2, opt-in gated; member grid + public profile view)
│       ├── groups/ + groups/[slug]/            # Alumni groups (community V2; list w/ cohort quick-join + detail w/ topics/events tabs)
│       ├── jobs/                 # Job board (community V2; broadcast read, cards + post dialog + report)
│       ├── announcements/        # Staff announcements (community V2; read-only, pinned first, ใหม่ badge via watermark)
│       ├── notifications/        # Notification center (community V2; unread first, mark read; linked from the header bell)
...
│   ├── alumni-upload/            # Opt-in-alumni image upload (shares `saveImageUpload` with /api/upload) — feeds the activity-feed photo attach + the community-profile photo
│   ├── community-profile/        # Own community profile (community V2) — GET/PUT upsert, no /id; `requireForumAlumni`. Empty strings normalize to null
│   ├── directory/ + directory/[id]/   # Alumni directory (community V2) — GET list (`search`/`cohort`/`degreeLevel`/`province`/`country`; newest-opt-in first) + one public profile (404 when not opted-in); `resolveForumReader`; directory-only identity select (`SELECT_ALUMNI_DIRECTORY_IDENTITY`)
│   ├── groups/ + groups/[slug]/ + groups/[slug]/membership/   # Alumni groups (community V2) — list/create + detail/manage + join/leave. Guards in `lib/group-guard.ts`; cohort materialization in `lib/group-cohort.ts`
│   ├── jobs/ + jobs/[id]/       # Job board (community V2) — broadcast-read list (`scope=active|expired|mine`) + author-or-staff PUT/DELETE. Creator pattern = events
│   ├── events/[id]/photos/ + events/[id]/photos/[photoId]/   # Event photo albums (community V2) — GET/POST register `{imageUrl, caption}` (ATTENDING/organizer/staff gate) + uploader-or-staff DELETE
│   ├── announcements/ + announcements/[id]/ + announcements/mark-read/   # Staff announcements (community V2) — broadcast GET (`?since=true` newCount vs watermark) + staff POST/PUT/DELETE + watermark stamp
│   ├── notifications/ + notifications/unread-count/   # Own notification center (community V2) — GET list (unread first) + POST mark-read `{ids}|{all}` + badge count. `getAlumniSession`
...
- **Deviations from the standard pattern:**
  ...
  - `logs` — read-only (GET list only; filters `resource`/`action`/`userId`/`role` — `role` = `superadmin|admin|executive` (→ actorType ADMIN + userRole) or `alumni|system` (→ actorType); replaced the old `source` param 2026-09) + a **superadmin-only** `POST /api/logs/bulk-delete` `{ ids }` that **hard-deletes** (and deliberately does NOT log the deletion). UI gate is `useRole() === "superadmin"` (NOT `useIsAdmin()`). The System Logs page highlights ADMIN actors with a role badge (`ACTOR_ROLE_LABELS`/`ACTOR_ROLE_COLORS` in `lib/log-detail.ts`, distinct color per role) next to the green ศิษย์เก่า / gray ระบบ actor-type badges. **All-Thai display contract:** `details` is rendered at READ time by `lib/log-detail.ts` — the SINGLE Thai-label surface (action/resource maps are `Record<LogAction/LogResource, string>` from client-safe `lib/log-types.ts`, so adding a LogAction/LogResource without a label is a compile error; a new `details` key must be added to `FIELD_LABELS` or `META_KEYS` (uuid/machine keys are hidden — log a sibling Thai title instead, e.g. `topicTitle`/`eventTitle`/`postSnippet`) and to the registry fixture in `tests/log-detail-labels.test.ts`; enum VALUES under `status`/`resourceType`/`mode`/`bulk`/`entity` need a Thai map entry in `formatValue`. Display surfaces (System Logs page, `AlumniActivityTimeline`, `FieldHistoryModal`) consume the shared maps — never duplicate a label map locally.
  ...
  - `alumni-upload` — opt-in-alumni image upload (`requireForumAlumni`). Shares `saveImageUpload` (`lib/upload.ts`) with the admin `/api/upload` so the 5 MB / PNG+JPG / magic-byte rules are identical; writes to the same `public/uploads`. Exists because `/api/upload` is admin-gated and alumni need to attach photos to feed posts (+ the V2 community-profile photo).
  - `community-profile` + `directory` — alumni community V2. `community-profile` = the logged-in opted-in alum's OWN self-published profile: GET (row or null) / PUT upsert (`requireForumAlumni`, no `/[id]` — mirrors `alumni-profile`; empty strings normalize to null so the form can clear; photo via `/api/alumni-upload`, stored on `CommunityProfile.photoUrl`, displayed as `profile.photoUrl ?? alumni.photoUrl`). `directory` = the opted-in member list: `GET /api/directory` (`search` over name/workplace/position + `cohort`/`degreeLevel`/`province`/`country` filters, newest-opt-in first) and `GET /api/directory/[id]` (one public profile; **404 when not opted-in** — opt-out hides). Both gate via `resolveForumReader` and select ONLY through `SELECT_ALUMNI_DIRECTORY_IDENTITY`. UI: `directory/` grid + `directory/[id]` view on the alumni portal; the self-profile form is the self-contained `CommunityProfileSection` card on `graduates/profile`. No import/export/bulk-delete, no admin page.
  - `groups` — community V2 groups/sub-communities. **A group's space = existing topics/events scoped by `groupId` — NOT a new feed.** Guards in `lib/group-guard.ts` (`resolveGroupReader` = staff OR opted-in alum; `requireGroupMember` = opted-in + membership row → 403 `{code:"NOT_A_MEMBER"}`; managing (PUT/DELETE) = staff via `checkWritePermission` or a MODERATOR; COHORT groups are read-only/auto-derived). Routes: `GET/POST /api/groups` (list with one batched `isMember`/`myRole`; POST creates INTEREST — `resolveEventCreator` pattern, creator auto-joins as MODERATOR; Thai-only titles get a hashed slug), `GET/PUT/DELETE /api/groups/[slug]` (param = slug OR id), `POST/DELETE /api/groups/[slug]/membership` (idempotent join; **`cohort:<label>` slug lazily materializes the group via `ensureCohortGroup` and is own-cohort-only (403 otherwise)**; leave decrements). **`groupId` extensions:** `GET /api/forum/topics?groupId=<id|none>` (the alumni forum page passes `groupId=none`; omitted = all), `POST /api/forum/topics {groupId}` (member-gated; `topicCount++` in the same `$transaction`; DELETE of a group topic decrements), `GET /api/events?groupId=`, `POST /api/events {groupId}` (alumni organizer must be a member). `memberCount`/`topicCount` denormalized; `community-group` in `TRASH_ENTITIES`. Seed: `scripts/seed-cohort-groups.ts` (`DRY_RUN=1`). No admin page (group content moderated via the report queue).
  - `jobs` — community V2 job board. **Broadcast read** (`resolveEventReader` — any ACTIVE alum; `scope=active|expired|mine`, default active hides expired), **creator-gated write** (`resolveEventCreator` — staff or opted-in alum; dual author FKs like events). `GET/POST /api/jobs`, `GET/PUT/DELETE /api/jobs/[id]` (PUT/DELETE = author-or-staff; soft-delete). Expiry: required `expiresAt` ≤ 90 days (zod `expiryRefine`); no cleanup cron — expired simply filters out. Reports: `resourceType:"JOB_POSTING"` (deleteUrl `/api/jobs/[id]` in the moderation queue). No admin page (reports-driven).
  - `events/[id]/photos` — community V2 event photo albums. Broadcast read (`resolveEventReader`); rows carry `canDelete` (uploader-or-staff) computed inline. POST registers an already-uploaded image (`{imageUrl, caption}` — bytes went through `/api/alumni-upload`); upload gate = ATTENDING alum / alumni organizer / staff (`lib/event-photo-gate.ts`, pure + tested) → 403 `{code:"NOT_ATTENDEE"}`; `communityRateLimit(upload)`. DELETE `[photoId]` = uploader-or-staff soft-delete (checks the alumni session directly so it survives opt-out). Reportable via `EVENT_PHOTO` (deleteUrl `/api/events/[id]/photos/[photoId]`). UI: `components/events/EventPhotoAlbum.tsx` mounted on the event detail page.
  - `announcements` — community V2 staff announcements. `GET` = broadcast read (`resolveEventReader`): non-expired, pinned first, `?since=true` adds `newCount` vs the caller's `Alumni.announcementsLastReadAt` watermark (comparison only — the watermark is stamped by `POST /api/announcements/mark-read` when the alumni page actually renders the list). `POST /api/announcements` + `GET/PUT/DELETE /api/announcements/[id]` = staff-only (`checkWritePermission`; exec read-only). DELETE soft-deletes (`announcement` in `TRASH_ENTITIES`). Admin page: `/management/announcements` (plain textarea CRUD + pin toggle). **Announcements ≠ News** — plain text, community audience, watermark badge; don't merge.
  - `notifications` — community V2 in-app notification center. `GET /api/notifications` (own, unread-first, `getAlumniSession`), `POST {ids?}|{all:true}` mark-read, `GET /api/notifications/unread-count` (one indexed count; polls every 60s from the header bell). **Emitters are best-effort side-effects** (`lib/notification-emitter.ts` — suppressed try/catch, never fail the parent action): reply→topic author, like/comment→post author (like only on LIKE, never un-like; self-actions skipped), RSVP→organizer (new ATTENDING only), group topic→members (**capped at `GROUP_NOTIFY_CAP=200`** — larger groups skip; one chunked `createMany`; announcements use the watermark INSTEAD), report resolve/dismiss→reporter. Thai title/body/link are PRE-RENDERED at emit (`lib/notification-text.ts`) — later source edits don't rewrite history; links are basePath-relative (the page prepends BASE_PATH). Purge: `/api/auth/cleanup` deletes read notifications > 90d (not logged).
...
| `framer-motion` | Product-tour animations (spotlight spring + tooltip transitions — the ONLY approved use; see Key Constraints exception) |
...
| `components/tour/*` | Product-tour framework — `TourProvider` (mounted in BOTH authed layouts; `useTour().start()`), the radix+framer-motion spotlight overlay, `TourHelpButton` (the header 'i'), registry + pure geometry in `lib/tours.ts` · `lib/tour-geometry.ts` (see the product-tour lesson) |
...
| `components/OrangeCell.tsx` · `FieldHistoryModal.tsx` | Orange update indicators + per-field change-history modal (radix-based) |
...
| `components/community/CommunityProfileSection.tsx` | Self-contained "โปรไฟล์ชุมชนศิษย์เก่า" card on `graduates/profile` (own edit state + photo upload + `PUT /api/community-profile`; join card when not opted in) |
...
- **Rule:** `basePath` is a **build-time** constant baked into the standalone image; `docker compose up -d` reuses an existing image and does NOT rebuild. After source changes the app serves old code (at `/` not `/alumni` → every internal link/fetch 404s). **Mode-switch variant (2026-09):** dev and prod builds share ONE tag (`fon-cmu-alumni-app`) — a prod build (`-f docker-compose.yml up --build`) retags it with the `runner` stage (standalone `node_modules`: no `.bin/next`, no `dotenv`), so the next plain `docker compose up` boots `dev-watch.mjs` against the standalone image and **crash-loops exit 127** (`sh: next: not found`, `Cannot find module 'dotenv/config'`) while the port stays published — `docker compose ps` still LOOKS live.
- **Prevention:** Run `docker compose up -d --build app` after any source change you want live. **On a dev↔prod mode switch (or whenever the app container is crash-looping after a rebuild), ALSO pass `--renew-anon-volumes`** — compose REUSES the previous container's anonymous volumes on recreate, so a `/app/node_modules` volume seeded from the wrong-stage image survives the rebuild until renewed. Verify with curl: `/` → 308 `/alumni`, `/alumni/login` → 200, bare `/login` → 404. `basePath` is NOT runtime-overridable, so a wrong basePath always means a stale build.
...
- **Prevention:** After any `prisma/schema.prisma` change: `migrate status` (drift check) → create → apply → `generate`. **Always run `migrate status` before migrating** — an abandoned worktree may have applied a migration missing from your local folder → Prisma offers `migrate reset` (DATA LOSS); copy the missing file in instead. **Never accept a reset** on the shared remote DB. Run DB scripts with `node --env-file=.env --import tsx`. **Non-interactive shells:** Prisma 7 `migrate dev` has no `--force` and aborts on destructive diffs (drops/enum-value removal) — hand-author `prisma/migrations/<UTC-ts>_<name>/migration.sql` (mirror the engine's SQL style; Postgres can't drop enum values, so recreate the type + `USING (col::text::_new)` cast, deleting rows holding removed labels FIRST) and apply with `migrate deploy`.
...
### CMU `level_id` → degree mapping is authoritative (user-corrected 2026-09)
- **Rule:** `0`=NURSING_ASSISTANT (ประกาศนียบัตรผู้ช่วยพยาบาล — unconditionally, NO `major_name_th` disambiguation), `1`=BACHELOR, `3`=MASTER, `5`=DOCTORAL, anything else (`2`, `4`, unknown, empty)=ASSOCIATE. Single source: `cmuLevelToDegree` (`lib/alumni-verify.ts`); `cmuLevelToEnum` (`lib/cmu-graduate-filters.ts`) is the filter-side twin — a parity test in `tests/alumni-verify.test.ts` pins them together. Degree is derived at READ time (`cmu_graduates` stores the raw `levelId`; no re-sync needed after a mapping change), but values MATERIALIZED into `Alumni`/`Education` snapshots need `scripts/fix-degree-level-mapping.ts` (`DRY_RUN=1` preview; overwrites only old-mapping-produced values, then `recomputePrimaryEducation`).
- **Prevention:** Don't "fix" the ids back or re-introduce a major-name check on `"0"` — the original ids were wrong and were corrected by the user. If the mapping ever changes again, run the fixup script against each environment.
...
- **All-alumni page specifics:** inline edit + row-click → `/management/alumni/[id]`. Rows are a CLIENT-SIDE CMU+local merge (see below). The page is FETCH-ONCE (`queryKeys.alumni.manageBase()`, param-free, staleTime 5min): one request pair loads the RAW CMU list (`dedupe=false`) + the full local list (`includeDeleted=true`, `pageSize=50000` — the old `9999` silently truncated ~1,600 records), then a `useMemo` pipeline runs **dedupe → filter → merge → sort** entirely client-side (`dedupeCmuGraduatesByPerson` → `applyCmuGraduateFilters`/`filterLocalAlumniRows` → `mergeAlumniTableRows` → `sortAlumni`). Sort/search/facet/dedupe-toggle/paging are INSTANT (no refetch); `alumni.all` invalidations still prefix-match the key (one refetch pair per mutation). Adding a sortable column must keep it reachable by `sortAlumni`; never page+merge per-page. **`Alumni.birthDate` is stored Buddhist-era DDMMYYYY (8 digits)** — normalize before sorting/date-math; `formatBirthDateThai`/`formatBirthDateThaiSlash` display it (Thai DD-MM-YYYY Buddhist, year +543). CMU `birthday` is raw DD-MM-YYYY → normalize via `normalizeCmuBirthday` (`lib/alumni-verify.ts`).
...
- **The merge:** the page's fetch-once `manageBase` query loads the raw CMU list + full local list, then a client-side `useMemo` pipeline filters/merges/sorts; it overlays local identity/contact, appends local-only alumni, and collapses multi-degree persons via the `student_ids` bridge. **Filtering is pure + shared, never duplicated:** CMU rows filter through `applyCmuGraduateFilters` (`lib/cmu-graduate-filters.ts`, client-safe) and local rows through `filterLocalAlumniRows` (`lib/alumni-local-filter.ts`) — the SAME functions `/api/alumni/export` runs server-side, so table + export can't drift (pinned by `tests/alumni-manage-pipeline.test.ts`; dedupe must run BEFORE filtering — the server route's order). Any new server-side "all alumni" consumer must call `mergeAlumniTableRows` (`lib/alumni-merge.ts`; `tests/alumni-merge.test.ts`), never `prisma.alumni.findMany` alone (misses ~9.5k CMU-only persons). Export threads the caller's `dedupe` mode.
...
- **Prevention:** Wrap read-heavy, rarely-changing payloads in `withTtlCache(key, ttlMs, fn)` (`lib/cache.ts`, server-only — module-level `Map`, persists per Node process; the app is `output: "standalone"` Docker, NOT serverless). Dashboard + alumni-count are cached 60s (TTL-only — no write-route busting; errors aren't cached). `getCmuGraduatesLocal()` (the ~20k-row table read behind `/api/cmu-alumni`, facets, typeahead, exports) is cached 60s under `"cmu-graduates-local"` and BUSTED by `materializeCmuGraduates`. Force freshness via `bustCache("dashboard")` / `bustCachePrefix("alumni")`.
...
- **There are exactly TWO identity fragments (V2):** `SELECT_ALUMNI_DIRECTORY_IDENTITY` (same file) extends the public one with `graduationYear` + the opted-in alum's SELF-PUBLISHED `CommunityProfile` — used ONLY by `/api/directory*`. Never let the directory fragment (or a raw `communityProfile` include) appear in forum/feed/report queries; the extended `CommunityProfile` columns reach the DB only through the directory routes. Both fragments' field sets are pinned by `tests/forum-identity.test.ts`.
...
### A11y/UI conventions — shared shells only; interactive markup rules
- **Rule:** Dialogs ALWAYS go through `components/ui/modal.tsx` (forms/content) or `components/confirm-dialog.tsx` (pure confirmations) — radix gives focus trap/restore, Escape, `role="dialog"` + accessible title, scroll lock. NEVER hand-roll a `fixed inset-0` overlay (the pre-2026-08 codebase had 43; all migrated). Sortable table headers are `<th scope="col" aria-sort={…}><button onClick={sort}>label+icon</button></th>` — never `onClick` on the `<th>` itself (mouse-only). Row-click navigation additionally wraps the primary cell's id in a real `<Link>` (keyboard reach); if that cell is OrangeCell-hot, the Link must be a SIBLING (`↗` with aria-label), not inside OrangeCell's `<button>` (nested interactive elements are invalid HTML). Icon-only buttons carry `aria-label` (Thai, same as `title`).
- **Prevention:** globals.css ships the a11y foundations (`:focus-visible` ring, `prefers-reduced-motion`, `.skip-link`, `.sticky-col`/`.sticky-col-head` for the trailing actions column of wide tables). `FormField` auto-associates label↔control + error aria when given a single element child (composite controls must forward `id`/`aria-*` to their trigger — see `BirthDateSelect`). Custom dropdowns close on `pointerdown` (touch) + Escape and expose `aria-expanded` (see `facet-filter.tsx`). RTL tests for radix components under happy-dom are flaky (Escape/unmount) — cover those manually in the browser.

### Narrow-viewport overflow is INVISIBLE under mobile emulation — sweep desktop mode too
- **Rule:** Chromium in **mobile emulation stretches the layout viewport** to the page's min-content width (innerWidth silently becomes 330/743/… and everything "fits"), while the same width in **desktop rendering** (DevTools responsive bar without a device preset, docked panels eating page width) shows real overflow: a page-wide horizontal scroll + a blank page-background band on the right. Mobile-only headless sweeps will report `overflow=0` and lie.
- **Prevention:** When verifying responsive layout, sweep BOTH `isMobile:true` and plain desktop contexts at 320/375/425 and assert `document.documentElement.scrollWidth === window.innerWidth`. Concrete trap (fixed 2026-08): the admin + alumni headers had a ~330px min-content — the `h-16` crest logo (≈193px wide) in a `shrink-0` wrapper plus fixed-width buttons; fixed with `h-10 sm:h-12 lg:h-16` + base `px-3`. The `top-22` sticky-sidebar offsets stay correct because the sidebars are `lg:block`, where the logo is `h-16` again.

### Wide tables leak min-content into the DOCUMENT scroll area — `contain: paint` fences it
- **Rule:** Chromium leaks a wide table's min-content width out of its `.overflow-x-auto` box into the document scrollable area — even THROUGH `overflow-hidden` ancestor cards (all-alumni: document scrollWidth 729 at a 320 viewport while every visible box was ≤304). Effect: swipable blank band on the right in desktop rendering; silent viewport-stretch on mobile.
- **Prevention:** `globals.css` puts `contain: paint` on `.overflow-x-auto:has(table)` (same rule as the drag-pan cursor). Verified compatible: `position: sticky` action columns still pin (`stickyRight == wrapper right`, even scrolled), drag-to-pan `scrollLeft` writes unaffected, radix dialogs portal outside the container. Do NOT replace it with `table-layout: fixed` (resizes columns) or `overflow: clip` (kills the scroll). If a new table page shows phantom page-level horizontal scroll, check this rule is applied (it's automatic via `:has(table)`).

### Product tours (spotlight walkthroughs) — data-driven, radix+framer, `data-tour` targeting
- **Rule:** The 'i'-button tutorial overlay (`components/tour/*`) is a **radix modal Dialog** (transparent Overlay = click blocker; spotlight hole = a `motion.div` with `0 0 0 9999px` box-shadow dim; tooltip card = `DialogPrimitive.Content asChild` onto `motion.div`) — NOT a hand-rolled `fixed inset-0` overlay, NOT the styled `Modal` (which hardcodes centering). **EVERY authed page now has a tour** (38 in `lib/tours.ts`); adding one = pure data: a `TourDefinition` + `data-tour="<page>-<el>"` attributes on the page (dual-mount the same id across loading/error/empty/table branches of list ternaries — one renders at a time; error-branch h1s too, so role-gated pages degrade to heading+closing). `TourProvider` is mounted in BOTH authed layouts; pages reach it via `useTour().start(tourId?)` (the profile first-login modal does). `tourForPath` matches `usePathname()` paths which are **basePath-stripped** — patterns stay plain (`/graduates/profile`).
- **Registry ordering is load-bearing:** `tourForPath` is `TOURS.find()` (first-match-wins; string patterns match exact-or-child). List child tours BEFORE their parents — `/page/new` (string) → `/page/[^/]+` (anchored RegExp `^…\/?$` for [id] pages) → `/page` (string parent); `alumni-profile` must stay at registry index 0 (`tests/tour-overlay.test.tsx` renders `TOURS[0]`). Pinned by shadowing invariants + a golden resolution table in `tests/tours.test.ts`.
- **Prevention:** (1) Every tour MUST keep ≥1 `target: null` centered step (test-enforced) so missing targets can't kill it — steps whose target never appears are SKIPPED after `waitFor ?? 3000ms` (start waits 8000ms for step 0 to cover react-query `isPending`). (2) Tours are **desktop-only v1**: the 'i' button + the modal's tour button are `hidden lg:*` — don't remove that or add mobile steps without authoring mobile targets. (3) `MotionConfig reducedMotion="user"` gates the springs (globals.css only zeroes CSS animations). (4) `onInteractOutside` preventDefaults so stray clicks on the dim don't end the tour — Escape/ข้าม only. (5) Completion flag = `tour-completed:<id>:<alumniId|role>` (alumni layout passes `userId`; admin scopes by role — no client user-id). (6) Rect state/setState only inside rAF/listener/observer callbacks (React Compiler lint). (7) **Spotlight rects are clamped to the visible viewport below the sticky header** (`clampRectToViewport`, fed the header's measured bottom from `useTargetRect`) and targets taller than the viewport scroll their TOP below the navbar (not `block:"center"` — centering hides the thead and the raw rect makes the navbar fall inside the highlight hole). (8) Modal-based creates (events/announcements/groups/jobs) target the TRIGGER button, never dialog content. Runtime locked by `tests/tour-overlay.test.tsx`; geometry/registry by `tests/tour-geometry.test.ts` + `tests/tours.test.ts` (Escape/focus-restore verified manually — radix under happy-dom is flaky there).
