...
- **Rule:** `docker compose up` auto-merges the override → builds the Dockerfile **`deps`** stage + bind-mounts the source (`.:/app`) for HMR; anonymous volumes protect `node_modules`/`.next`/`app/generated`.
- **Prevention:** On the first switch to dev (or after any `package.json` change) you MUST `docker compose up -d --build app` once. New host-side npm deps update host `node_modules` but NOT the container's (anonymous volume) → dev server 500s `Module not found` while host `npm run build`/`lint` still pass. Fix: `docker compose exec app npm install`.
...
- **Prevention:** (1) restart the dev server; (2) if 500s or selective 404s persist, **`rm -rf .next` then restart** (a plain restart does NOT clear Turbopack's persistent cache). Verify the data layer independently with a `node --env-file=.env --import tsx` script and confirm `npm run build` + `npm run test` pass before blaming the code. **Docker dev container:** `app/generated/prisma` is an anonymous volume and the running process holds the old client — run `prisma generate` **inside** the container (`docker compose exec app npx prisma generate`) AND `docker compose restart app`. For an additive nullable column: `migrate status` → `migrate dev --create-only` → `migrate deploy` → `generate`; never `migrate reset`.
