...
| `MentorProfile` | `mentor_profiles` | V2 mentor volunteer — 1:1 with `Alumni` (`alumniId @unique`), `expertise`, `capacity` (1–10), `accepting` flag (pause without losing history). Opt-in gated |
| `MentorshipRequest` | `mentorship_requests` | A mentee's request to a mentor — `MentorshipStatus` PENDING→ACCEPTED/DECLINED (mentor) or CANCELLED (mentee). One PENDING per (mentee, mentor). **The accept-response payload is the ONLY place mentor contact info (contactEmail/email + phones) leaves the DB** |
...
│       ├── mentorship/           # Mentorship (community V2; tabs หาพี่เลี้ยง / คำขอของฉัน / เป็นพี่เลี้ยง)
...
│   ├── mentors/ + mentorship-requests/ + mentorship-requests/[id]/   # Mentorship (community V2) — volunteer list/upsert (`?me=true` own) + requests GET/POST + `{action:accept|decline|cancel}`. Transition rules in `lib/mentorship-flow.ts`
...
- **Deviations from the standard pattern:**
  ...
  - `mentors` + `mentorship-requests` — community V2 light mentorship. **Opt-in gated** (`resolveForumReader`/`requireForumAlumni`). `GET /api/mentors` (list w/ open-request count via one groupBy; `?all=true` includes paused; `?me=true` own) / `PUT` upsert own (capacity 1–10, `accepting` pause flag). `GET /api/mentorship-requests` (sent + received, public identity only) / `POST` (guards: not accepting, self, at-capacity ACCEPTED count, duplicate own-PENDING — 400 with Thai messages). `POST /api/mentorship-requests/[id] {action:"accept"|"decline"|"cancel"}` — transition rules single-sourced in `lib/mentorship-flow.ts` (accept/decline = mentor, cancel = mentee, PENDING only). **The accept response is the ONLY contact disclosure** — it carries the mentor's `contactEmail ?? email` + `phones`; never selected anywhere else. No matching algorithm — the mentee browses.
