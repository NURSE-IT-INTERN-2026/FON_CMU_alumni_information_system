...
- **What:** Every admin management table page (all-alumni, awards, associations, graduate-committee, potentials, news, alumni-agency, model-representatives) renders ONE table with CRUD always on — checkbox column, แก้ไข/ลบ actions, เพิ่มข้อมูล, import/export, bulk-select, OrangeCell hot-field indicators. The old `manageMode` state and the "จัดการข้อมูล" / "กลับหน้าเดิม" toggle are gone.
...
### all-alumni: single merged CMU+local CRUD table (CMU birthday normalized client-side)
