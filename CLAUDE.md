...
- `ContentReportStatus`: OPEN, RESOLVED, DISMISSED · `ContentReportReason`: SPAM, HARASSMENT, INAPPROPRIATE, OTHER · `ForumReportResource`: FORUM_TOPIC, FORUM_REPLY, EVENT, FEED_POST, FEED_COMMENT (one report system across forum + events + feed) · `RsvpStatus`: ATTENDING, DECLINED (community events) · `GroupKind`: COHORT, INTEREST · `GroupRole`: MEMBER, MODERATOR (V2 groups)
