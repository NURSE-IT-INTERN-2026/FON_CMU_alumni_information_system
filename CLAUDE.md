...
- **Prevention:** The `/api/cmu-alumni` list proxy trims `student_id` in its response — the all-alumni table + `useAlumniSearch` consume that (already-trimmed) output and need no trimming of their own. If you consume `fetchCmuGraduates()` DIRECTLY (a lib/server route), you MUST `.trim()` the id before comparing or keying. Never drop the proxy's trim. (Residual ~312 row-vs-person gap vs the dashboard is a DIFFERENT cause — multi-degree alumni whose *primary* `studentId` isn't in CMU but a *secondary* degree is; the table keys on the primary only, the dashboard unions all degrees.)
