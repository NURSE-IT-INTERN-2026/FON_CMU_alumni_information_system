...
│   ├── cmu-alumni/               # CMU Registrar list/search proxy (GET only) + /lookup?studentId=&alumniId= (GET — single-record auto-fill preview for the add-education form; returns samePersonWarning when alumniId is a different person). The list `GET` accepts `?dedupe=false` to SKIP `dedupeCmuGraduatesByPerson` (the all-alumni "แยกรายการตามรายวุฒิ" toggle — list every degree record instead of one row per person; default `true`). The alumni `GET /api/filter-facets` mirrors it so facet counts match.
...
- **Prevention:** The dashboard counting path now uses `fetchCmuGraduatesOrEmpty()` (`lib/cmu-registrar.ts`) — wraps `fetchCmuGraduates()`, catches ANY error (non-2xx, timeout, network) → `{ graduates: [], available: false }`, logs the reason. `groupPersonsByDegree([], local)` then counts LOCAL persons only (still meaningful; ~10,871 vs the usual ~20,988 merged total). `getPersonDegreeBreakdown` returns `cmuAvailable: boolean`; both `/api/dashboard` and `/api/alumni-count` echo it in their JSON; the dashboard page renders an amber "ไม่สามารถเชื่อมต่อระบบทะเบียน (CMU) ได้" banner (`!cmuAvailable`) explaining the totals are local-only. A failure is NOT cached, so it self-heals within the 60s dashboard TTL once CMU is back (the route's own `withTtlCache` already rate-limits CMU to ~1 hit/60s during an outage). **Strict CMU surfaces MUST keep the throwing `fetchCmuGraduates()` / `fetchCmuGraduateById()`** — `/api/cmu-alumni` (proxy), `/api/cmu-alumni/sync`, profile lookup (`/api/alumni/[id]`), `ensure-alumni` (import), the all-alumni table, and signup/approve/reverify identity checks — hiding the outage there would silently serve empty/wrong data. Don't blanket-fail-safe at the fetch layer; only the dashboard COUNT degrades.
