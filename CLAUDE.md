...
│       ├── cmu-sync/             # "การดึงข้อมูล" — compare local cmu_graduates vs remote CMU + ดึงข้อมูล button (admin+superadmin)
...
- **`cmuAvailable` re-pointed:** previously "Registrar reachable"; now "the `cmu_graduates` table is non-empty". Pre-first-sync → `false` → the dashboard amber banner says "ยังไม่ได้ดึงข้อมูล CMU" (links to `/management/cmu-sync`) and counts local alumni only.
