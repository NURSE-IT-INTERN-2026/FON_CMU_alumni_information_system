...
- **Selection is opt-in (a "เลือก" toggle, not always visible):** a header "เลือก" button enters select mode — reveals the checkbox column (select-all `<th>` + per-row checkbox) — and "เสร็จสิ้น" exits it (clears selection via `deselectAll`). In select mode, clicking a row/card body **toggles selection** (navigation paused); the checkbox and in-row buttons still work because the row `onClick` guards with `closest("button, input, a")` (table pages) or the cell `stopPropagation` (all-alumni) / `<Link onClick={preventDefault}>` (news cards). Each page holds `selectMode` + `enterSelect`/`exitSelect`. The bulk buttons (ลบ/ส่งออกที่เลือก) stay gated on `selectedCount > 0`, which is only reachable in select mode. Per-page notes: alumni-agency's "เลือก" button is abroad-only (the Thailand tab has no selection); all-alumni's checkbox `<th>`/`<td>` had no `{(` wrapper (it lived in the always-shown manage table) so they're wrapped in `{selectMode && (...)}` rather than flipping an existing one.
