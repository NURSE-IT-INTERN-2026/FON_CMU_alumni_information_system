...
- **Admin alumni profile view:** `/management/alumni/[id]` (param = alumni UUID **or** `studentId` — `GET /api/alumni/[id]` resolves either) is reached by clicking any row on the all-alumni + alumni-related tables; its data-logs tab reads `GET /api/alumni/[id]/activity` (field-change history ∪ activity log). Editing there saves via `PUT /api/alumni/update-with-related/[id]` (overlaps the full-form route; sets `adminEditedAt`). Replaces the deleted `/management/settings/alumni/[id]`. Orange indicators on this page query BOTH `resourceType: alumni` and `alumni_profile` (admin + alumni-self edits) — `OrangeCell`/`FieldHistoryModal`/`/api/field-changes` accept comma-joined `resourceType`. The **alumni-self** profile (`/graduates/(authed)/profile`) shows the SAME orange indicators (personal + contact fields) via `useHotFields("alumni,alumni_profile")`.
- **Alumni import name-change logging:** `POST /api/alumni/import` reads `นามสกุลเดิม` (required; falls back to legacy `นามสกุล` header) + `นามสกุลใหม่` (optional). When a row has a new last name ≠ old, the import writes the record with the NEW name and records the change as a `lastName` `FieldChangeHistory` (resourceType `alumni`) + an `UPDATE` `ActivityLog` (reason `นามสกุลใหม่จากการอิมพอร์ต`) — so the field shows an orange indicator on BOTH the admin and alumni-self profiles, and the old name is preserved in the edit history (one-way: it's a logged edit, not trash-recoverable). Idempotent on re-import (skips when the stored name already equals the new). `scripts/build-alumni-excel.ts` emits both columns from `Tbl_temp` `TLNAME`/`TLNAME_NEW`.
...
- **Why:** `currentWorkplace`/`country`/`province` were dropped from `Alumni`; `maidenLastName`+`newLastName` merged into `lastName`. Admin uses `HotInfoField`/`OrangeCell` (orange edit indicators) on the personal + contact fields; alumni-self ALSO uses `OrangeCell` (via `useHotFields("alumni,alumni_profile")`) on those same fields — so an imported name change (see `POST /api/alumni/import`) shows up orange + clickable for the alumni too, not just admins. `homeAddress` (existed on the schema, previously only used by `AlumniAgency`) is now the alumni contact address — wired into `profileFormSchema`/create-update-with-related/`alumni-profile`/forms.
