 ...
 *     (from the /graduates area it appends &audience=alumni to open the alumni tab);
...
describe("apiFetch 401 redirect", () => {
  ...
  it("opens the alumni login tab when expired in the graduates area", async () => {
    (window.location as { pathname: string }).pathname = "/alumni/graduates/profile";
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(fakeResponse(401, { error: "กรุณาเข้าสู่ระบบ" })),
    );
    const { apiFetch } = await import("@/lib/api-client");
    await expect(apiFetch("/api/alumni-profile")).rejects.toThrow();
    expect(navigatedTo).toBe("/alumni/login?expired=1&audience=alumni");
  });
