...
describe("CMU Email API client", () => {
  ...
  it("prefers PUBLIC_BASE_URL over NEXT_PUBLIC_BASE_URL for email links", async () => {
    process.env.PUBLIC_BASE_URL = "https://alumni.nurse.cmu.ac.th";
    const calls = mockFetch();
    const { sendSignupApprovedEmail } = await import("@/lib/email");
    await sendSignupApprovedEmail("a@cmu.ac.th", "คุณ สมชาย");
    const sendBody = calls.at(-1).body as Record<string, string>;
  ...
  it("validateBaseUrl: dev allows localhost, prod rejects it, prod accepts a real URL", async () => {
    const { validateBaseUrl } = await import("@/lib/base-url");
    // dev/test env (NODE_ENV !== "production"): localhost is fine
    expect(validateBaseUrl().ok).toBe(true);

    const prev = process.env.NODE_ENV;
    process.env.NODE_ENV = "production";
    try {
      // localhost → not ok
      const bad = validateBaseUrl();
      expect(bad.ok).toBe(false);
      if (!bad.ok) expect(bad.reason).toMatch(/PUBLIC_BASE_URL/);
  ...
  it("in production with a localhost base URL, sendEmail throws before calling fetch", async () => {
    const prev = process.env.NODE_ENV;
    process.env.NODE_ENV = "production";
    // no PUBLIC_BASE_URL → falls back to NEXT_PUBLIC_BASE_URL=localhost
    try {
      const spy = vi.spyOn(globalThis, "fetch");
      const { sendSignupApprovedEmail } = await import("@/lib/email");
      await expect(sendSignupApprovedEmail("a@cmu.ac.th", "A")).rejects.toThrow(
